...
3. 前端生产环境的 `VITE_API_BASE_URL` 已设为 `/api`，或确实配置成了你需要的完整 API 地址
4. Nginx 配置文件中的 `server_name` 已替换为真实域名
5. HTTPS 证书路径已替换为真实域名对应路径，且证书文件真实存在
6. `root` 路径与真实部署目录一致，且不要放在 `/root` 这类受限目录下
7. Nginx 用户对静态目录及其父目录具有读取/遍历权限
8. `sudo nginx -t` 校验通过
9. `http://你的域名` 会自动跳转到 `https://你的域名`
10. `https://你的域名` 页面首页可正常打开
11. `/api/health` 通过 HTTPS 域名访问正常
12. 前端页面实际操作时 `/api/*` 请求无异常，且不再出现写死的 `127.0.0.1:3001`
